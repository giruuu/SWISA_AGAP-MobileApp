...
class HomeScreen extends StatelessWidget {
        ...
        child: Column(
          children: [
            const Padding(
              ...
              child: CustomHeader(), // <-- Use the new reusable header
            ),
            Expanded(
              child: ListView(
  ...
  // The old _buildHeader method has been removed, as it's now in custom_header.dart
  ...
  Widget _buildFeaturesSection() {
    return IntrinsicHeight(
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Expanded(child: _buildFeatureButton(iconColor: Colors.blue.shade700, icon: Icons.edit_document, label: 'APPLY GRANT')),
          const SizedBox(width: 14),
          Expanded(child: _buildFeatureButton(iconColor: AppColors.accentYellow, icon: Icons.calendar_today, label: 'TRAINING &\nWORKSHOP')),
          const SizedBox(width: 14),
          Expanded(child: _buildFeatureButton(iconColor: Colors.pink.shade400, icon: Icons.checklist, label: 'VIEW STATUS')),
        ],
      ),
    );
  }

  Widget _buildFeatureButton({required Color iconColor, required IconData icon, required String label}) {
    return Container(
